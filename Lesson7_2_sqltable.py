...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        uic.loadUi('Lesson7_2_sqltable.ui', self)
        self.table = self.tableView
         
        database = r"/Users/guoyixuan/Documents/pythoncode/ccwdatabase/database.sqlite"
        # create a database connection
        self.conn = create_connection(database)
        with self.conn: # with can handle the exceptions, like resources released, cleaning...
            select_table(self, "Authors")
        #有出錯時可以幫忙清理現場(with self.conn)
    ...
    # Slots
    def queryTable(self,i): #查詢表格
        tbname = self.select_table.currentText()
        select_table(self, tbname) #給他名稱
